...
A C++98 dynamically resizing array template `DynamicallySizedArray<T>` implemented without STL containers. Provides contiguous storage with index-based access and vector-like operations.
...
- **Safety**: Index operations assert bounds in debug builds (via `<cassert>`).
...
## Tests
Official tests are on Kattis. This README focuses on local compilation and running the interactive driver with the sample inputs.
